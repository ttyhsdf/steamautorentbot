...
from integration.enhanced_bot import EnhancedAutoRentSteam, set_enhanced_bot
from integration.chat_sync_integration import initialize_chat_sync_integration, cleanup_chat_sync_integration
...
def main_loop():
    ...
    try:
        ...
        # Инициализируем расширенный бот с Chat Sync
        print("🧩 Инициализация Chat Sync плагина...")
        try:
            config = {
                'CHAT_SYNC_ENABLED': True,
                'MASTER_ENCRYPTION_KEY': None,  # Не обязателен для Chat Sync
                'PAYMENT_SYSTEM_ENABLED': False,
                'USER_MANAGEMENT_ENABLED': False,  # Отключаем для упрощения
                'PLAYWRIGHT_ENABLED': False
            }
            enhanced_bot = EnhancedAutoRentSteam(config)
            set_enhanced_bot(enhanced_bot)  # Устанавливаем глобальный экземпляр
            print("✅ Chat Sync плагин инициализирован")
        except Exception as e:
            logger.warning(f"Ошибка инициализации Chat Sync: {str(e)}")
            print(f"⚠️ Chat Sync недоступен: {str(e)}")
        ...
        if enhanced_bot and enhanced_bot.chat_sync:
            print("🧩 Chat Sync плагин активен")
        ...
        try:
            if enhanced_bot:
                ...
                print("✅ Chat Sync плагин остановлен")
        except Exception as e:
            logger.error(f"Error stopping Chat Sync: {str(e)}")
